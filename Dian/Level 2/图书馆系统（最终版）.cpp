...
void administrator()
{
    ...
    while(cin >> ten)
    {
        ...
        else if(ten == "Cancel")
        {
            ...
            cin >> x >> y;
            tu[tem][flo][x][y] = 0;
            for(int i=0;i<ans[goal].size();i++)
            {
                if(Day[ans[goal][i].day] == tian && ans[goal][i].floor == flo && ans[goal][i].x == x && ans[goal][i].y == y)
                {
                    ans[goal].erase(ans[goal].begin()+i);
                }
            }
            update();
        }
...
int main()
{
    ...
    while(true)
    {
        ...
        else if(book == 1 && s == "Administrator")
        {
            administrator();
        }
        else
